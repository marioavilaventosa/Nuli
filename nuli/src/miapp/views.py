...
from .models import Editar_Perfil, Entrenamiento, Frase, Año
        ...
        for dia in entrenamiento.dias.all():
            if ahora == dia.nombre:
                ...
                for ejercicio in dia.ejercicios.all():
                    ejercicios.append(ejercicio.nombre)
                    ejercicios_foto.append(ejercicio.foto)
        context = {
            "frase": frase,
            "ejercicios": ejercicios,
            "ejercicios_foto": ejercicios_foto,
            "descanso": descanso,
...
#Hay que hacerlo con cada entrenamiento
#Actualiza el entrenamiento del usuario
def Entrenamiento_Fat_Burn(request):
    usuario = request.user
    entrenamiento = Entrenamiento.objects.filter(nombre='Entrenamiento Fat Burn').get()
    Editar_Perfil.objects.filter(usuario=usuario).update(ejercicios=entrenamiento)
    return redirect('/')

def Entrenamiento_Fuera_Camiseta(request):
    usuario = request.user
    entrenamiento = Entrenamiento.objects.filter(nombre='Entrenamiento Fuera Camiseta').get()
    Editar_Perfil.objects.filter(usuario=usuario).update(ejercicios=entrenamiento)
    return redirect('/')

def Entrenamiento_Full_Body(request):
    usuario = request.user
    entrenamiento = Entrenamiento.objects.filter(nombre='Entrenamiento Full Body').get()
    Editar_Perfil.objects.filter(usuario=usuario).update(ejercicios=entrenamiento)
    return redirect('/')